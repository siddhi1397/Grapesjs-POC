...
export class WebBuilderComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      pluginsOpts: {
        ...
        'gjs-plugin-ckeditor': {
          ...
          options: {
            ...
            extraPlugins:
              'sharedspace,justify,colorbutton,panelbutton,justify,font,floatpanel,panel,openlink,richcombo,strinsert,mentions,xml,autocomplete,textmatch,textwatcher,ajax',
    ...
    this.editor.on('component:selected', (args: any) => {
      if (args.attributes.type === 'wrapper') {
        ...
        args.attributes.droppable = true;
        ...
        // args.attributes.dmode = 'absolute';
        // args.attributes.textable = 1;
      }
      // args.attributes.dmode = 'absolute';

      args.attributes.dmode = 'translate';
    });

    // let abc = document.querySelector('iframe')?.contentWindow?.document;
    // console.log(document.querySelector('iframe')?.contentWindow?.document);

    // for (let i = 0; i < abc!.length; i++) {
    //   console.log(abc![i]);

    //   abc![i].addEventListener('drop', (e: any) => {
    //     console.log('hi');

    //     e.dataTransfer?.getData('text/plain');
    //     console.log(e);

    //     // $(e.dataTransfer?.getData('text/plain')).css(
    //     //   'left',
    //     //   Number(e.clientX) + 20
    //     // );
    //     // $(e.dataTransfer?.getData('text/plain')).css(
  ...
  submit() {
    console.log(
      juice(`${this.editor.getHtml()}<style>${this.editor.getCss()}</style>`)
    );
  }
